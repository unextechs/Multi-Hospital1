...
// custom scrollbar (only if niceScroll is available)
...
// } else {
//     console.warn('NiceScroll library not loaded, using default scrollbars');
// }
...
/**
 * Global Select2 focus fix for Bootstrap Modals
 * Resolves the issue where Select2 search input is not clickable/focusable inside modals.
 */
if ($.fn.modal && $.fn.modal.Constructor && $.fn.modal.Constructor.prototype._enforceFocus) {
    var conf = $.fn.modal.Constructor.prototype._enforceFocus;
    $.fn.modal.Constructor.prototype._enforceFocus = function() {
        var that = this;
        $(document).on('focusin.modal', function(e) {
            var modalElement = (that.$element && that.$element[0]) || that._element;
            if (modalElement) {
                ...
                if (modalElement !== e.target && !$(modalElement).has(e.target).length) {
                    if (that.$element) {
                        that.$element.focus();
                    } else if (that._element && typeof that._element.focus === 'function') {
                        that._element.focus();
                    }
                }
            }
