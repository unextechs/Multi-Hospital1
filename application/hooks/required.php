...
function required()
{
    ...
    if ($RTR->class != "frontend" && $RTR->class != "payu" && $RTR->class != "status" && $RTR->class != "cronjobs" && $RTR->class != "request" && $RTR->class != "auth" && $RTR->class != "site" && $RTR->class != "api" && strtolower($RTR->class) != "tv") {
    ...
    if ($CI->router->fetch_class() == 'site' && $CI->router->fetch_method() == 'index') {
        ...
        if ($RTR->class != "cronjobs" && $RTR->class != "frontend" && $RTR->class != "payu" && $RTR->class != "status" && $RTR->class != "request" && $RTR->class != "auth" && $RTR->class != "api" && strtolower($RTR->class) != "tv") {
            if (!$CI->ion_auth->in_group(array('superadmin'))) {
        ...
        if ($RTR->class != "cronjobs" && $RTR->class != "payu" && $RTR->class != "status" && $RTR->class != "frontend" && $RTR->class != "request" && $RTR->class != "auth" && $RTR->class != "api" && strtolower($RTR->class) != "tv") {
            if (!$CI->ion_auth->in_group(array('superadmin'))) {
                if ($CI->ion_auth->in_group(array('admin'))) {
                    $current_user_id = $CI->ion_auth->user()->row()->id;
                    ...
                    $group_modules_row = $CI->db->get_where('hospital_group_modules', array('hospital_id' => $hospital_id, 'group_id' => $group_id))->row();
        ...
        $common = array('payu', 'status', 'macro', 'auth', 'pservice', 'settings', 'import', 'home', 'profile', 'request', 'api', 'cronjobs', 'logs', 'doctorvisit', 'site', 'testpkz', 'facilitie', 'faq', 'diagnosis', 'treatment', 'symptom', 'advice', 'inventory', 'emergency', 'ambulance', 'dashboard', 'radiology', 'dental', 'role_permission', 'frontend', 'hl7', 'dicom', 'tv');

        if (!in_array(strtolower($RTR->class), $common)) {
            if (!$CI->ion_auth->in_group(array('superadmin'))) {
                if ($RTR->class != "schedule" && $RTR->class != "meeting" && $RTR->class != "featured" && $RTR->class != "gallery" && $RTR->class != "review" && $RTR->class != "gridsection" && $RTR->class != "service" && $RTR->class != "slide" && $RTR->class != "facilitie" && $RTR->class != "faq") {
                    if ($RTR->class != "pgateway") {
                        if (!in_array(strtolower($RTR->class), $CI->modules)) {
                            redirect('home');
                        }
                ...
                if (!in_array(strtolower($RTR->class), $CI->super_modules)) {
                    redirect('home');
                }
            }
        }
