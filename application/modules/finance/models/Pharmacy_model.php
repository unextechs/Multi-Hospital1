...
class Pharmacy_model extends CI_model {
    ...
    function getPaymentItems($payment_id) {
        $payment = $this->getPaymentById($payment_id);
        if (!$payment || empty($payment->category_name)) {
            return array();
        }

        $items = array();
        $category_items = explode(',', $payment->category_name);
        ...
        foreach ($category_items as $item) {
            ...
            if (count($parts) >= 4) {
                ...
                $medicine = $this->db->get_where('medicine', array('id' => $medicine_id))->row();
